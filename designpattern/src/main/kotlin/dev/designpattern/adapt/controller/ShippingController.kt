package dev.designpattern.adapt.controller
...
class ShippingController {
    ...
    @PostMapping("/quote")
    fun quote(@RequestBody req: QuoteRequest): ResponseEntity<Any> {
        return try {
            when (req.carrier.uppercase()) {
                "SHIPFAST" -> shipFastQuote(req)
                "QUICKSHIP" -> quickShipQuote(req)
                else -> ResponseEntity.badRequest().body(
                    mapOf("error" to "unsupported carrier: ${req.carrier}")
                )
            }
        } catch (e: Exception) {
