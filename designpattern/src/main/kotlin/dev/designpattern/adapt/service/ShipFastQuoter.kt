...
import dev.designpattern.adapt.common.constant.SHIP_FAST_BASE
...
class ShipFastQuoter: ShippingQuoter {
    override fun quote(request: QuoteRequest): QuoteResponse =
        runCatching {
            val url = URI.create("$SHIP_FAST_BASE/rate")
            ...
            val resp = RestTemplate().postForEntity(url, http, ShipFastResponse::class.java).body
